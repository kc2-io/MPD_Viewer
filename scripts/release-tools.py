...
def publish() -> None:
    ...
    notes.write_text(f"MPD Viewer `{tag}` from `{head}`.\n\n" + platforms + "\n\n"
                     ...
                     "Adds MPD logo/colors and a unified Windows Twitch connection flow with the fixed "
                     "application ID. Fixes activation links, authorization redirects and return navigation. "
                     "The connection window closes after authorization and return navigation complete. "
                     "Per-channel chat and the earlier player startup fix are retained.\n\n"
                     "Windows activation, shared website-session recognition and restart persistence were "
                     "user-confirmed. Automatic window closing passed code/lifecycle checks but still awaits "
                     "a complete user acceptance test. API authorization remains in memory; reconnect after "
                     "quitting. Existing preferences and the viewer browser profile are preserved.\n\n"
