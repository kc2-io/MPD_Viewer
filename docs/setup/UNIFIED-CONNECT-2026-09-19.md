...
## Automatic connection-window close

The user confirmed that the return-navigation fix completed activation successfully. The native diagnostic also recorded fixed markers for the authorization frontend and the exact validated return; it recorded no query values or credentials.

At the user's request, the connection window now closes automatically after both successful API authorization and a native Finished event for the exact validated return URL. The read-only matcher preserves the existing activation-code guard. The native callback sends only the attempt epoch through the existing internal channel, with no new remote IPC capability. Either signal may arrive first; both must belong to the current tracked attempt. Disconnect clears completion state, stale events are ignored, and duplicate events cannot close a later window. A close failure preserves the connected credentials and tracked window and shows a manual-close message.

Tauri/WebView2 Finished means navigation completed, not proof every resource loaded successfully. API authorization remains the authoritative connection signal; this change makes no additional claim about website identity, Turbo or player recognition.

Verification: 32 Rust workspace tests and the optimized Windows build passed. Independent review approved. An isolated native fixture using the actual controller, a separate app identity and in-memory preferences passed cancellation before reply (including a stale AuthReturnLoaded event), stale API replies, repeated Connect, malformed-link abort and Demo rejection, exit 0. These synthetic lifecycle tests do not establish real automatic closing; that user test remains pending. No release, tag or website deployment.
