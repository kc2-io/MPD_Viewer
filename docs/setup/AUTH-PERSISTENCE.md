...
`cargo test --locked -p mpd-twitch` and `cargo clippy --locked -p mpd-twitch --all-targets -- -D warnings` pass using an isolated target directory. Controller integration now starts asynchronous restoration before accepting Start, retains the same in-memory session across transient failures with bounded backoff, retries initial secure-save failures, invalidates old epochs before disconnect/reconnect, and checks idle authentication maintenance every minute (the OAuth adapter performs hourly network validation). `cargo test --locked -p mpd-tabber --features custom-protocol` passed 34 native Windows tests on the integration base. Four injected async worker tests cover missing credentials, transient/rotated session retention, initial save failure, and terminal/old-generation results. Real user authorization followed by multiple full Windows app restarts passed in the combined build; the connected account restored without an authorization popup. Explicit user-credential deletion/restart and offline-start/recovery remain unobserved; deletion and recovery have the isolated native/deterministic evidence above. macOS/Linux persistence is not implemented.
