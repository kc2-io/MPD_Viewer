...
try:
    with sync_playwright() as p:
        ...
        check('Playback summary card is removed',page.locator('#playing-heading').count()==0)
        check('Ranking has no up/down arrow buttons',page.get_by_role('button',name=re.compile(r'^Move .* (up|down)$')).count()==0)
        check('Player sessions are above the summary',page.locator('.workspace-panel').bounding_box()['y'] < page.locator('.metrics').bounding_box()['y'])
        check('Rescan defaults to one minute with an accessible hint',page.locator('#rescan').input_value()=='1' and page.locator('#rescan').get_attribute('aria-describedby')=='rescan-hint')
        (ROOT/'docs').mkdir(exist_ok=True)
        page.screenshot(path=str(ROOT/'docs/ui-preview.png'),full_page=True)
        page.evaluate('window.__fixture.settings.demo=false')
        page.wait_for_function('document.querySelector("#monitor-heading").textContent==="Every 1 min"')
        page.locator('#rescan').fill('10')
        page.locator('#rescan').press('Enter')
        page.wait_for_function('window.__actions.some(a=>a.type==="set_rescan")')
        check('Rescan input dispatches whole minutes',page.evaluate('window.__actions.find(a=>a.type==="set_rescan")')=={'type':'set_rescan','minutes':10})
        page.evaluate('window.__fixture.settings.demo=true')
